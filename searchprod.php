...
<div class="box-section">
    ...
    <form action="searchprod.php" method="get" class="d-flex" role="search">
      <input class="form-control me-3" type="search" name="search" placeholder="Search" aria-label="Search">
      <button class="btn btn-outline-primary" name="search_btn" type="submit">Search</button>
    </form>
    </div>
<?php
$link = new mysqli('localhost','root','','douzo');
...
$sql = "SELECT * FROM tbl_rec WHERE Prod_name LIKE '%".$search_var."%'";
        ...
        <thead>
            ...
            <th class="table-dark"><h6>Product ID</h6></th>
                <th class="table-dark"><h6>Product Name</h6></th>
                <th class="table-dark"><h6>Product Price</h6></th>
                <th class="table-dark"><h6>Product Expiration</h6></th>
        ...
        <tbody>
            ...
            ?>
                        <tr>
                            ...
                            <td><p><?php echo $row['Prod_price'];?></p></td>
                            <td><p><?php echo $row['Prod_exp'];?></p></td>
                            <td><p><?php echo $row['Category'];?></p></td>
                            <?php 
                            if($row['Quantity'] <30){
                            echo"<td><p style='border-left:3px solid red;font-size:13px;font-style:italic; color:red;'>".$row['Quantity']." low in stocks</p></td>";
                            }else{
                                echo"<td><p>".$row['Quantity']."</p></td>";
                            }
                            
                            echo "<td>
                             <a class='btn btn-primary' href='./update/update_record.php?id=".$row['Prod_Id']."'>update</a>
                             <a class='btn btn-danger' href='record_management.php?id=".$row['Prod_Id']."'>Delete</a>
            ...
            ?>
                    <tr>
                        <td colspan="7"><p>Not Found</p><?php echo $link->error;?></td>
                    </tr>   
            <?php       
                }
            ?>
